...
Claude Code에서 QueenBee에 자료를 등록하는 **stdio MCP 서버**입니다.
(`FR-CLI-001`, [DEV-08 자료 수집 파이프라인](../../../_docs/02.개발설계/08_자료수집_파이프라인.md))

이 패키지는 **개발자 PC에서 돕니다.** QueenBee 서버에 배포되는 것이 아니라
Claude Code가 자식 프로세스로 실행하는 클라이언트입니다.

## 1. API 키 발급

QueenBee 웹에서 **마이페이지 > 보안 > API 키 > 키 발급**.
...
`skills/queenbee-collect/` 를 `~/.claude/skills/` 로 복사합니다.
...
| 도구 | 용도 | 필요 스코프 |
| --- | --- | --- |
| `queenbee_list_content_types` | 콘텐츠 타입 목록과 타입별 필드 스키마 | `resources:read` |
| `queenbee_search` | 기존 자료 검색 | `resources:read` |
| `queenbee_get_resource` | 자료 상세 | `resources:read` |
| `queenbee_check_duplicate` | URL 정규화 후 중복 확인 | `resources:read` |
| `queenbee_create_resource` | 자료 등록 | `resources:write` |
| `queenbee_update_resource` | 자료 보강 | `resources:write` |
| `queenbee_list_taxonomy` | 카테고리 트리·인기 태그 | `resources:read` |
| `queenbee_archive_github` | GitHub 소스 아카이브 요청 | `archive:run` |

## 개발

```bash
npm run --workspace @queenbee/mcp build     # dist/ 생성
...
| 타입 목록을 하드코딩하지 않는다 | `queenbee_list_content_types` 로 서버에서 받아온다 |
