...
/**
 * `queenbee_search` — 기존 자료 검색 (`FR-CLI-003`).
 *
 * **등록 전에 부르는 도구입니다.** 「이미 있는가」를 URL 로 묻는 것은
 * `queenbee_check_duplicate` 이고, **주제로** 묻는 것이 이쪽입니다 — 같은
...
export const search: ToolDef<Args> = {
  name: "queenbee_search",
  title: "자료 검색",
  description:
    "QueenBee 에 이미 있는 자료를 키워드·타입·태그·카테고리로 검색한다. 등록하기 전에 비슷한 자료가 있는지 확인할 때 쓴다. searchTruncated 가 true 면 결과가 잘린 것이므로 '없다'고 판단하지 말고 질의를 좁혀 다시 부른다.",
  inputSchema: {
    ...
    type: z
      ...
      .optional()
      .describe(
        "콘텐츠 타입 코드. queenbee_list_content_types 가 알려 주는 값만 쓴다"
      ),
