...
const envSchema = z.object({
  ...
  /**
   * 네오에게 **브라우저**를 준다 (`1` 이면 켜짐). **기본은 꺼져 있습니다.**
   *
   * ## 왜 스위치인가
   *
   ...
   * ## 무엇이 위험한가
   *
   * 네오가 연 페이지의 글이 **명령처럼 읽힐 수 있습니다**(prompt injection).
   * 그때 네오는 우리 자료 검색 도구를 함께 쥐고 있습니다. 그래서 켜더라도
  ...
  /**
   * 네오가 열 수 있는 주소를 **이 목록으로 한정**합니다 (세미콜론 구분).
   *
   ...
   * 네오가 무엇을 하는지 눈으로 봅니다 — 「진짜 여는지」를 확인하거나, 막힌
   * 곳에서 손으로 이어받을 때 씁니다.
   *
  ...
  /**
   * 네오의 브라우저 서버가 **듣는 포트**. 기본 `3101`.
   *
   * ## 왜 포트가 생겼는가
   *
   * 처음에는 질문마다 브라우저 서버를 새로 띄웠습니다. 그러면 답이 끝날 때
   * 브라우저도 같이 죽어서 **창이 10초쯤 깜빡이고 사라집니다** — 운영자가
   * 「브라우저를 못 띄우는데?」라고 한 이유입니다.
   *
   * 지금은 **한 번 띄워 두고 계속 씁니다.** 창이 남아 있어 네오가 클릭하는
   * 것을 그대로 볼 수 있습니다.
   *
   * ## 대가
   *
   * - **이 PC 안에서만** 듣습니다(`localhost`). 사내망에는 안 열립니다.
   *   그래도 이 PC 를 쓰는 다른 프로그램은 그 브라우저를 조종할 수 있습니다
   * - **상태가 남습니다.** 앞 질문에서 연 탭이 다음 질문에 그대로 있습니다.
   *   프로필은 메모리에만 있으므로(`--isolated`) 서버를 다시 띄우면 초기화됩니다
   */
  CHAT_BROWSER_PORT: z
    .string()
    .optional()
  ...
  /**
   * 네오의 **목소리** — Supertonic 모델 폴더 (`DEC-072`).
   *
