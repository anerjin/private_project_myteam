 ...
 * ## 관리자도 못 봅니다
 *
 * 🔄 이 시스템의 다른 곳은 대개 «로그인한 사람 전원»에게 전부 보여 줍니다
 * (`DEC-077` 로 등급이 사라진 뒤로는 더 그렇습니다 — 초안도, 남의 컬렉션도).
 * 여기는 **아닙니다.** 운영자가 「나만 보는 노트」라고 정했고, 예외를 하나 두면
 * 그건 「나만」이 아닙니다.
 *
 * 그래서 이 파일에는 `Actor` 가 아니라 **`ownerId` 만** 들어옵니다. 등급이
 * 있던 시절에는 「역할을 알면 언젠가 역할로 분기하게 된다」가 그 이유였고,
 * 등급이 없어진 지금은 **`Actor` 를 받는 순간 「전원에게 열린 것」들과 같은
 * 모양이 되기 때문**입니다 — `ownerId` 만 받으면 그럴 수가 없습니다.
 *
 * ## 감사 로그를 남기지 않습니다
 *
