...
| 항목 | 값 |
| --- | --- |
| 루트 경로 | `STORAGE_ROOT` — 기본 `E:\queenbee-data\files` (`DEC-016`). `tmp/`·`attachments/`·`archives/` 생성 완료 |
...
| `DATABASE_URL` | `postgresql://queenbee:…@localhost:**5433**/queenbee` | Prisma 접속. 포트는 `DEC-033` |
| `REDIS_URL` | `redis://localhost:**6380**` | `DEC-033` |
| `SESSION_COOKIE_NAME` | `qb_session` | 세션 쿠키 이름 (`DEC-030`) |
...
| DB 볼륨 | `E:\queenbee-data\postgres` 대신 **named volume**(`queenbee-dev_queenbee-pg`)을 씁니다. 백업은 `pg_dump` 로 받습니다 (`NFR-BACKUP-001`) |
