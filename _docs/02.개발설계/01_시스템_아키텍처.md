...
| `SESSION_TTL_DAYS` | `14` | 세션 만료. «로그인 상태 유지» 미체크 시 브라우저 세션 쿠키 |
...
>
> **Redis 클라이언트는 `enableOfflineQueue: false` 로 둡니다.** 기본값(`true`)이면 연결이 끊긴 동안
> 명령을 큐에 쌓아 재연결까지 기다리므로, `NFR-AVAIL-004`(캐시 미스로 처리하고 DB 폴백)가
> 성립하지 않고 요청이 통째로 매달립니다.
