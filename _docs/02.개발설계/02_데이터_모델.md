...
| 종류 | 대상 |
| --- | --- |
| INDEX | `(url_normalized)` — 부분 인덱스로 `deleted_at IS NULL` 일 때만. **유니크가 아니다** (`DEC-047`): `FR-RES-011` 이 「그래도 등록」을 수용 기준으로 두므로 DB 가 막으면 안 된다. 중복 정책은 진입점에 있다(웹=허용 · MCP=`409`) |
| UNIQUE | `(slug)` |
...
CREATE UNIQUE INDEX idx_resources_url_unique
  ON resources (url_normalized)  -- UNIQUE 아님 (DEC-047)
