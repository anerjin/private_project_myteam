...
sequenceDiagram
    ...
    SVC->>DB: INSERT (source_channel=MCP, needs_review=true)
...
{ "id": "clx…", "url": "http://localhost:3100/resources/ai-material/rag-…",
  "needsReview": true, "queuedJobs": ["FETCH_URL_META"] }
```
...
| 한 번에 등록량 | 한 대화에서 10건을 넘기지 않는다. 검수 부담이 커진다 |
...
에이전트가 만든 자료는 **`needs_review = true`** 로 등록됩니다.

| 항목 | 내용 |
| --- | --- |
| 표시 | 목록·상세에 `검수 대기` 배지 |
| 필터 | 자료 목록과 관리자 화면에서 `검수 대기` 필터 제공 |
| 검수 | `EDITOR` 이상이 확인 후 해제. 등록자 본인도 자기 자료는 해제 가능 |
| 검수 항목 | 요약이 원문과 맞는가 · 사내 적용 아이디어가 말이 되는가 · 분류가 맞는가 |
| 미검수 자료 | 숨기지 않는다. 배지만 붙인다 — 검색은 되어야 가치가 있다 |

> **왜 자동 게시하는가:** 승인 대기 큐를 만들면 아무도 비우지 않아 자료가 고입니다.
> 바로 보이게 하되 "아직 사람이 안 봤다"는 표시만 남기는 편이 실제로 운영됩니다.
